...
export function useReporteFinal() {
  ...
  // Bloqueo de seguridad de inicialización
  useEffect(() => {
  ...
  // Sincronizar estadísticas desde la configuración en la carga inicial
  useEffect(() => {
  ...
  // Inicializar novedades manuales por defecto si están vacías
  useEffect(() => {
  ...
  const obtenerFechaOrdenamiento = (novedad: Report | NovedadManual): Date | null => {
  ...
  const manejarAgregarNovedadManual = () => {
    ...
    if (idEditandoManual) {
      saveSettings({
        finalReportManualNovedades: novedadesManuales.map(n => 
  ...
  const manejarEliminarNovedadManual = (idParaEliminar: string) => {
    saveSettings({
      finalReportManualNovedades: novedadesManuales.filter((n) => n.id !== idParaEliminar)
    });
  };
    ...
    const contenidoReporte = todasNovedadesOrdenadas
      .map((item) => {
        if (item.type === 'report') {
          ...
          const novedad = item.data as NovedadManual;
          const sortDate = item.sortDate!;
          const fechaFormateada = new Intl.DateTimeFormat('es-ES', {
            day: '2-digit',
            month: '2-digit',
  ...
  const manejarCopiarReporte = () => {
    if (reporteGuardadoSeleccionado) {
