...
export default function AttendancePage() {
    ...
    const dailyRecords = useMemo(() => getRecordsByDate(formattedDate), [getRecordsByDate, formattedDate]);
        ...
        <div className="flex-1 flex flex-col space-y-6 p-8 bg-slate-50/50 min-h-screen">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Registro diario del personal y estadísticas de cumplimiento.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <Card key={i} className="border-none shadow-sm overflow-hidden bg-white/50 backdrop-blur-sm group hover:shadow-md transition-all">
                        <CardContent className="p-6">
                            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{stat.label}</p>
                            <div className="flex items-baseline gap-2 mt-2">
                                <span className={cn("text-5xl font-bold tracking-tighter", stat.color)}>{stat.value}</span>
                            </div>
                            <p className="text-[10px] text-muted-foreground mt-2 font-bold uppercase tracking-tight">{stat.sub}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <Card className="lg:col-span-8 border-none shadow-sm bg-white/50 backdrop-blur-sm">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-6 border-b border-slate-100/50">
                        <div>
                            <CardTitle className="text-xl font-extrabold tracking-tight">Asistencia de Hoy</CardTitle>
                            <p className="text-sm font-medium text-muted-foreground capitalize mt-1">{displayDate}</p>
                        </div>
                    ...
                    <CardContent className="p-0">
                        <div className="divide-y divide-slate-100/50">
                            {filteredPersonnel.map((member) => {
                                ...
                                return (
                                    <div key={member.id} className="flex items-center justify-between p-5 hover:bg-slate-50/50 transition-all group">
                                        <div className="flex items-center gap-5">
                                            <div className={cn(
                                                "h-12 w-12 rounded-2xl flex items-center justify-center border-2 transition-all duration-500",
                                                status ? "bg-white border-primary/20 shadow-sm" : "bg-muted/30 border-transparent"
                                            )}>
                                                {Config ? <Config.icon className={cn("h-6 w-6", Config.color.split(' ')[1])} /> : <UserCircle className="h-6 w-6 text-muted-foreground/50" />}
                                            </div>
                                            <div>
                                        ...
                                        <div className="flex items-center gap-10">
                                            <div className="text-right flex flex-col items-end gap-1">
                                                <p className="text-[10px] font-extrabold text-muted-foreground/60 uppercase tracking-widest">Entrada</p>
                                                <p className="text-sm font-mono font-bold text-slate-700">{record?.checkInTime || '--:--'}</p>
                                            </div>
                ...
                <div className="lg:col-span-4 space-y-6">
                    <Card className="border-none shadow-sm bg-white/50 backdrop-blur-sm">
                        <CardHeader className="pb-2">
                            ...
                            <p className="text-xs font-medium text-muted-foreground">Filtra el historial por fecha</p>
                        </CardHeader>
                    ...
                    <Card className="border-none shadow-sm bg-primary/5 border border-primary/10">
                        <CardContent className="p-4 space-y-3">
                            <Button variant="outline" className="w-full justify-start gap-3 h-12 text-sm font-extrabold bg-white hover:bg-slate-50 border-slate-200/60 shadow-sm transition-all">
