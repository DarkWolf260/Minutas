    ...
    return (
      <div className="flex flex-col h-full bg-background overflow-y-auto px-4 pb-32">
        <SyncSetup 
          ...
          whatsappComponent={<SyncWhatsApp />} 
        />
        <SyncModals 
          hook={hook} 
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <ScrollArea className="flex-1" type="always">
        <div className={cn(
          "w-full px-4 pt-8 pb-32 space-y-8 animate-in fade-in duration-500 mx-auto",
          hook.esPrincipal ? "max-w-5xl" : "max-w-3xl"
        )}>
          <SyncHeader 
          ...
          {hook.esPrincipal ? (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mt-6">
              {/* Lado Izquierdo: Estado, QR, WhatsApp y Zona de Peligro (7 cols) */}
              ...
              {/* Lado Derecho: Bandeja de Entrada (5 cols) */}
              <div className="lg:col-span-5">
                <SyncInbox hook={hook} />
              </div>
            </div>
          ) : (
            /* Dispositivo Secundario: Una sola columna centrada */
