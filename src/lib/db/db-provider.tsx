    ...
    useEffect(() => {
        ...
        async function migrateData(database: MinutasDatabase) {
            ...
            if (settingsData) {
                ...
                } catch (err) { console.error('Settings migration failed:', err); }
            }
            ...
            if (unitsData) {
                ...
                } catch (err) { console.error('Units migration failed:', err); }
            }
            ...
            if (fieldsData) {
                try {
                    ...
                    const entries = Object.entries(parsed).map(([id, config]) => ({ id, config: config as any }));
                    if (entries.length > 0) {
                        await database.field_definitions.bulkInsert(entries);
                    }
                } catch (err) { console.error('Field definitions migration failed:', err); }
            }
            ...
            if (draftData) {
                ...
                } catch (err) { console.error('Draft migration failed:', err); }
            }
            ...
            if (templateConfigsData) {
                ...
                } catch (err) { console.error('Template configs migration failed:', err); }
            }
