...
export function parseTemplate(templateContent: string): { sections: SectionConfig[], layout: string[], fieldNames: Set<string> } {
    try {
        ...
        let lastIndex = 0;
        let openBracketIndex = templateContent.indexOf('[');
        
        while (openBracketIndex !== -1) {
            contentWithPlaceholders += templateContent.substring(lastIndex, openBracketIndex);
            
            let balance = 1;
            let closeBracketIndex = openBracketIndex + 1;
            while (balance > 0 && closeBracketIndex < templateContent.length) {
                const char = templateContent[closeBracketIndex];
                if (char === '[') {
                    balance++;
                } else if (char === ']') {
                    balance--;
            ...
            if (balance === 0) {
                const sectionBlock = templateContent.substring(openBracketIndex, closeBracketIndex);
                const placeholder = `__SECTION_${sectionIndex}__`;
                sectionMap.set(placeholder, sectionBlock);
                contentWithPlaceholders += placeholder;
                sectionIndex++;
            ...
            openBracketIndex = templateContent.indexOf('[', lastIndex);
        }
        
        contentWithPlaceholders += templateContent.substring(lastIndex);
        ...
        return { sections: [], layout: [], fieldNames: new Set<string>() };
    }
}
 ...
 * @returns The populated report string, or an error message on failure.
 */
        ...
        finalContent = finalContent.replace(/(\[(?:[^[\]]+|\[[^[\]]*\])*\])/g, (match) => {
            ...
            if (sectionConfig.isRepeatable) {
                ...
                const itemsContent = sectionData.map((item: Record<string, any>, index: number) => {
                    ...
                    // No subtitle for simple repeatable sections
                    else if (!isComplex && sectionConfig.repeatableItemLabel) {
                    }
                ...
                // For simple sections, join with a space only if there's more than one item.
                const separator = (isComplex || sectionData.length > 1) ? '\n\n' : '\n';
                
                return generatedBlock ? generatedBlock + '\n' + itemsContent.join(separator) : itemsContent.join(separator);
            } 
