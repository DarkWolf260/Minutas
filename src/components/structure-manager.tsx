...
export function StructureManager({
  ...
  const handleLoadInstitutional = async () => {
    if (
      !window.confirm(
        '¿Estás seguro de cargar la estructura institucional? Esto añadirá los departamentos y cargos estándar (no eliminará los actuales).'
      )
    )
      return;
    ...
    const institutionalRoles: StaffRole[] = [
      { name: LEADER_ROLES.DIRECTOR, isSingle: true, departmentScope: [], isHidden: false },
      {
        ...
        isSingle: true,
        departmentScope: [deptMap['ops']].filter(Boolean) as string[],
        isHidden: false,
      },
      {
        name: 'Jefe de los Servicios',
        isSingle: true,
        departmentScope: [deptMap['ops']].filter(Boolean) as string[],
        isHidden: false,
      },
      {
        name: 'Analista de CEMUPRAD',
        isSingle: false,
        departmentScope: [deptMap['cemuprad']].filter(Boolean) as string[],
        isHidden: false,
      },
      {
    ...
    institutionalRoles.forEach((r) => {
      const existingIndex = newRoles.findIndex(
        (ext) => ext.name.toLowerCase() === r.name.toLowerCase()
      );
      if (existingIndex === -1) {
  ...
  const handleAssignDept = async (roleName: string, deptId: string) => {
    const newRoles = roles.map((r) =>
      r.name === roleName ? { ...r, departmentScope: deptId === 'global' ? [] : [deptId] } : r
    );
    await onRolesChange(newRoles);
  };
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={handleLoadInstitutional}
                    ...
                    <div className="col-span-2 flex justify-center">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          handleUpdateRole(role.name, { isSingle: checked })
                        }
                        className="scale-75"
                      />
                    </div>
        ...
        <CardContent className="p-0">
          ...
          >
            <div className="divide-y divide-primary/5">
              <SortableContext
                items={roles.filter((r) => !r.isHidden).map((r) => r.name)}
                  ...
                  .map((role, idx) => (
                    <SortableRoleItem
                      ...
                      onRemoveFromList={async () => {
                        const newRoles = roles.map((r) =>
                          r.name === role.name ? { ...r, isHidden: true } : r
                        );
                        await onRolesChange(newRoles);
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing p-1 text-primary/40 hover:text-primary transition-colors"
        >
      ...
      <div className="flex items-center gap-3">
        <Badge
          variant="secondary"
          className="text-[10px] font-mono tabular-nums bg-primary/10 text-primary border-primary/20"
        >
          #{index + 1}
        </Badge>
        <Button
