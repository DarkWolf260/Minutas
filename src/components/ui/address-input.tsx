...
export function AddressInput({ value, onChange, placeholder, disabled }: AddressInputProps) {
    ...
    const filteredAddresses = useMemo(() => {
        ...
        const isAFormattedAddress = addresses.some(addr => formatAddressToString(addr).toLowerCase() === lowercasedValue);
                    ...
                    {addresses.length > 0 ? (
                        filteredAddresses.map(address => (
                            ...
                            >
                                ...
                                <span className="text-xs text-muted-foreground">{address.municipality}, {address.parish}</span>
                            </Button>
                        ))
                    ) : (
